/**
 * ui/Renderer.js
 * Módulo de renderizado dinámico del DOM.
 * Separa completamente la lógica de presentación de las clases de negocio.
 */
const Renderer = {
  ...
  /**
   * Crea una tarjeta de producto (Bicicleta) para el catálogo.
   * @param {Bicicleta} bicicleta
   * @param {Function} onAgregar - Callback al hacer click en "Agregar"
   * @returns {HTMLElement}
   */
  crearTarjetaProducto(bicicleta, onAgregar) {
    ...
    const badgeHTML = bicicleta.badge
      ? `<div class="absolute top-4 left-4 bg-primary text-on-primary px-3 py-1 font-label-bold text-label-bold text-xs">${bicicleta.badge}</div>`
      : '';
    ...
    card.innerHTML = `
      ...
      <div class="p-6 bg-surface-container-high flex-1 flex flex-col justify-between">
        <div>
          ...
          <h3 class="font-headline-md text-headline-md text-on-surface mb-2">${bicicleta.nombre}</h3>
        ...
        <div class="mt-4 flex items-center justify-between">
          <span class="font-headline-md text-xl text-primary font-bold">${bicicleta.precioFormateado}</span>
          <button 
            class="btn-agregar bg-primary text-on-primary px-5 py-3 font-label-bold text-sm flex items-center gap-2 hover:bg-white hover:text-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    btn.addEventListener('click', () => onAgregar(bicicleta));
  ...
  /**
   * Crea un item para la vista del carrito (sc.html).
   * @param {{ bicicleta: Bicicleta, cantidad: number }} item
   * @param {Function} onEliminar - Callback al eliminar
   * @param {Function} onCambiarCantidad - Callback al cambiar cantidad
   * @returns {HTMLElement}
   */
  crearItemCarrito(item, onEliminar, onCambiarCantidad) {
    ...
    div.innerHTML = `
      ...
      <div class="flex-grow space-y-2 w-full">
        <div class="flex justify-between items-start">
          <div>
            <h3 class="font-headline-md text-headline-md text-on-surface">${bicicleta.nombre}</h3>
            <p class="text-on-surface-variant font-label-bold text-sm">${bicicleta.categoria.charAt(0).toUpperCase() + bicicleta.categoria.slice(1)}</p>
          </div>
          <span class="font-headline-md text-xl text-primary">${bicicleta.precioFormateado}</span>
        </div>
        <div class="flex items-center justify-between mt-4">
          <div class="flex items-center border border-outline-variant bg-surface-container-low">
            ...
            <input class="qty-input w-12 text-center bg-transparent border-none focus:ring-0 font-label-bold" 
          ...
          <button class="btn-eliminar flex items-center text-on-surface-variant hover:text-red-400 transition-colors uppercase font-label-bold tracking-widest text-xs">
    ...
    div.querySelector('.qty-minus').addEventListener('click', () => {
      onCambiarCantidad(bicicleta.id, cantidad - 1);
    });
  ...
  mostrarToast(mensaje, tipo = 'success') {
    ...
    toast.className = `fixed bottom-6 right-6 z-[9999] px-6 py-4 font-label-bold text-sm uppercase tracking-wider shadow-2xl transition-all duration-300 ${colores[tipo]}`;
  ...
  actualizarResumenCarrito(compra) {
    ...
    if (subtotalEl) subtotalEl.textContent = fmt(compra.calcularSubtotal());
    if (descuentoEl) descuentoEl.textContent = `-${fmt(compra.calcularDescuento())}`;
    if (impuestosEl) impuestosEl.textContent = fmt(compra.calcularImpuestos());
    if (envioEl) envioEl.textContent = compra.calcularEnvio() === 0 ? '¡Gratis!' : fmt(compra.calcularEnvio());
    if (totalEl) totalEl.textContent = fmt(compra.calcularTotal());
    if (itemsEl) itemsEl.textContent = `${compra.totalItems()} artículo${compra.totalItems() !== 1 ? 's' : ''} en tu carrito.`;
   ...
   * @param {Function} onLogin - Callback al hacer login exitoso
   */
  mostrarModalSesion(onLogin) {
    ...
    overlay.innerHTML = `
      <div class="bg-surface-container border border-outline-variant w-full max-w-md p-8 relative">
        ...
        <h2 class="font-headline-lg text-primary uppercase tracking-tighter mb-2">Iniciar Sesión</h2>
        <p class="text-on-surface-variant text-sm mb-8">Accede a tu cuenta o continúa como invitado.</p>
        ...
        <div class="space-y-3">
          <button id="btn-login" class="w-full py-4 bg-primary text-on-primary font-headline-md uppercase tracking-widest hover:bg-white hover:text-black transition-all duration-300">
            Entrar
          </button>
          <button id="btn-invitado" class="w-full py-4 border border-outline-variant text-on-surface-variant font-label-bold uppercase tracking-widest hover:border-primary hover:text-primary transition-all duration-300">
    ...
    // Invitado
    overlay.querySelector('#btn-invitado').addEventListener('click', () => {
      overlay.remove();
      Renderer.mostrarToast('Continuando como invitado', 'info');
    });
  }
};
